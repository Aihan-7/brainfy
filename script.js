...
function renderSyncChip() {
    ...
    switch (syncState) {
        case 'idle':
            label = idToken ? 'Idle' : 'Local only';
            break;
        case 'syncing':
            label = 'Syncing…';
            break;
        case 'synced':
            label = lastSyncedAt ? `Synced ${timeSince(lastSyncedAt)}` : 'Synced';
            break;
        case 'error':
...
window.addEventListener('online', () => { if (syncState === 'offline')
    setSyncState(idToken ? 'idle' : 'idle'); });
...
function hydrateIcons(root = document) {
    root.querySelectorAll('.ms').forEach(hydrateOneIcon);
}
        ...
        for (const m of mutations) {
            m.addedNodes.forEach(node => {
                if (!(node instanceof HTMLElement))
                    return;
                if (node.classList && node.classList.contains('ms'))
                    hydrateOneIcon(node);
...
let _cfmResolver = null;
let _prmResolver = null;
let _prmFields = [];
function showConfirm(opts = {}) {
    return new Promise(resolve => {
        _cfmResolver = resolve;
        const modal = el('cfmModal');
        const titleEl = el('cfmTitle');
        const msgEl = el('cfmMessage');
        const confirmBtn = el('cfmConfirmBtn');
        const cancelBtn = el('cfmCancelBtn');
...
function showPrompt(opts) {
    return new Promise(resolve => {
        ...
        const modal = el('prmModal');
        const titleEl = el('prmTitle');
        const msgEl = el('prmMessage');
        const f1 = el('prmField1');
        const f2 = el('prmField2');
        const okBtn = el('prmOkBtn');
        if (!modal || !titleEl || !msgEl || !f1 || !f2 || !okBtn) {
            resolve(null);
            return;
        }
...
window.cfmDismiss = cfmDismiss;
window.prmDismiss = prmDismiss;
window.prmSubmit = prmSubmit;
...
function uploadFileToStorage(file, subjectId) {
...
function addDocToSubject(doc, subjectIdOverride) {
...
function openDoc(subjId, docId) {
    ...
    if (d.type === 'link') {
        ...
        if (!href) {
            showToast('File missing — not available locally', 'warning');
            return;
        }
...
function initEvents() {
    ...
    el('syncStatus')?.addEventListener('click', () => {
        if (syncState === 'error')
            retrySync();
        if (syncState === 'offline')
            showToast('You appear to be offline. Changes are saved locally.', 'info');
    });
    // ── Splash: View Pricing ───────────────────────────
...
function renderTTGrid() {
    ...
    TT_HOURS.forEach((h, hi) => {
        const label = h === 0 ? '12 AM'
            : h < 12 ? `${h} AM`
                : h === 12 ? '12 PM'
                    : `${h - 12} PM`;
        bgParts.push(`<div style="position:absolute;left:0;top:${hi * TT_ROW_H}px;width:52px;height:${TT_ROW_H}px;` +
        ...
        for (let di = 0; di < 7; di++) {
            bgParts.push(`<div style="position:absolute;` +
    ...
    const blocksHtml = S.timetable.map((b) => {
        ...
        if (clampedEnd <= 0 || clampedStart >= visibleMins)
        ...
        return (`<div onclick="openTTModal(${b.id})" ` +
    ...
    if (targetScroll > 0 && grid.scrollTop === 0)
...
function openTTModal(id) {
    ...
    if (!modal || !titleEl || !deleteBtn)
        return;
    const titleInp = el('ttBlockTitle');
    const dayInp = el('ttBlockDay');
    const startInp = el('ttBlockStart');
    const endInp = el('ttBlockEnd');
    if (id != null) {
...
function saveTTBlock() {
    ...
    if (!start || !end) {
        showToast('Please pick a start and end time', 'error');
        return;
    }
    if (start >= end) {
    ...
    if (ttEditId != null) {
        ...
        if (idx !== -1)
            S.timetable[idx] = { id: ttEditId, title, day, start, end, color: ttSelectedColor };
    }
    else {
...
function renderSettings() {
    ...
    if (fd)
        fd.value = String(Math.round(S.focusDuration / 60));
    if (bd)
        bd.value = String(Math.round(S.breakDuration / 60));
    if (dg)
        dg.value = String(S.dailyGoal);
}
let _settingsDirtyTimer;
function settingsCommitDirty() {
